class PostsController < ApplicationController
    ...
    def approve
        authorize @post
        @post.approved!
        redirect_to root_path, notice: "Post nb: #{@post.id} from: #{@post.user.full_name}, has been approved"
    end

    private

       def post_params
           params.require(:post).permit(:date, :rationale, :id, :status, :overtime_request)
       end
